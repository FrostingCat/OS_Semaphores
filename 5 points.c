...
int main(int argc, char* argv[]) {
    ...
    if ((cashier_1 = sem_open(SEM_NAME_1, O_CREAT, 0600, 0)) == SEM_FAILED) {
        perror("sem_open");
        return -1;
    }
    if ((cashier_2 = sem_open(SEM_NAME_2, O_CREAT, 0600, 0)) == SEM_FAILED) {
        perror("sem_open");
        return -1;
    }
    if ((queue = sem_open(SEM_NAME_3, O_CREAT, 0600, 1)) == SEM_FAILED) {
        perror("sem_open");
        return -1;
    }
    if ((for_1 = sem_open(SEM_NAME_4, O_CREAT, 0600, 0)) == SEM_FAILED) {
        perror("sem_open");
        return -1;
    }
    if ((for_2 = sem_open(SEM_NAME_5, O_CREAT, 0600, 0)) == SEM_FAILED) {
        perror("sem_open");
        return -1;
    }
